...
%Finally, we show that the CVD-$L_{2D}$ relation is not sensitive to the cloud Mach number.
\end{abstract}
...
%(see the review by \cite{Hennebelle2012} and the reference therein).
...
%and the gas velocity dispersion (Mach number)
...
%The $^{12}$CO map traces most of the CO gas, while the $^{13}$CO map traces relatively denser regions.
...
%In the second approach we use hydrodynamic simulations of molecular clouds to investigate the relation between the CVD-$L_{2D}(l_{3D})$ relation and the cloud Mach number.
...
%In order to examine the relation between the core velocities, which selectively probes dense gas, and the distributed cloud gas, we also analyzed the $^{12}$CO and $^{13}$CO point-to-point velocities following a similar procedure. We randomly selected individual pixels from the $^{12}$CO(1-0) and $^{13}$CO(1-0) observational data cubes (see section \ref{sec:data}). The velocity of each pixel was determined using the first moment of the spectrum at each pixel. The velocity dispersion was then obtained with similar technique as that of CVD (see Figure~\ref{fig:vdis_all_gas}). We found a relation between velocity dispersion $\sigma_v$ ($\equiv \langle\delta v^2\rangle^{1/2}$) and 2D length scale, $ \sigma_v$ (km/s)$=(0.6\pm 0.1)L_{2D} ({\rm pc})^{0.5\pm 0.04}+(0.4\pm 0.1)$ ($L_{2D}<1.5$ pc) and $ \sigma_v$ (km/s)$=(0.4\pm 0.03)L_{2D} ({\rm pc})^{0.3\pm 0.01}+(0.7\pm 0.02)$ (1.5 pc$<L_{2D}<10$ pc) for $^{12}$CO.
%For $^{13}$CO(1-0) the relation can be fitted with a broken power law with $ \sigma_v$ (km/s)$=(0.7\pm 0.02)L_{2D} ({\rm pc})^{0.5\pm 0.03}-(0.01\pm 0.02)$ ($L_{2D}<1.5$ pc) and $ \sigma_v$ (km/s)$=(0.8\pm 0.02)L_{2D} ({\rm pc})^{0.2\pm 0.01}+(0.01\pm 0.01)$ (1.5 pc$<L_{2D}<10$ pc).
%\cite{Blitz1997} found a scale of 0.25 pc to 0.5 pc where gravity become important. The break point of 1.5 pc found here may indicate that this is also the scale where gravity begins to dominate in Taurus.
...
%\centering
%\includegraphics[width=8cm]{vdis_all_gas_all.eps}
%\includegraphics[width=8cm]{vdis_all_gas_all13.eps}
%\end{minipage}
%\caption{The velocity dispersion of pixels randomly selected in  the $^{12}$CO(1-0) ({\it left}) and  $^{13}$CO(1-0) ({\it right}) data cube of the Taurus molecular cloud. The velocity dispersion vs. $L$ relation can be fitted with a broken power law with $ \sigma_v$ (km/s)$=(0.6\pm 0.1)L_{2D} ({\rm pc})^{0.5\pm 0.04}+(0.4\pm 0.1)$ ($L_{2D}<1.5$ pc) and $ \sigma_v$ (km/s)$=(0.4\pm 0.03)L_{2D} ({\rm pc})^{0.3\pm 0.01}+(0.7\pm 0.02)$ (1.5 pc$<L_{2D}<10$ pc), for $^{12}$CO(1-0). For $^{13}$CO(1-0), it can be fitted with a broken power law with $ \sigma_v$ (km/s)$=(0.7\pm 0.02)L_{2D} ({\rm pc})^{0.5\pm 0.03}-(0.01\pm 0.02)$ ($L_{2D}<1.5$ pc) and $ \sigma_v$ (km/s)$=(0.8\pm 0.02)L_{2D} ({\rm pc})^{0.2\pm 0.01}+(0.01\pm 0.01)$ (1.5 pc$<L_{2D}<10$ pc). The power law index is smaller than that of the CVD-$L$ relation in Figure \ref{fig:vdis_all}.}  \label{fig:vdis_all_gas}
%\end{figure*}
...
%A similar break can be seen in a model where cores are distributed in two clusters, each having a 7 pc radius and with a separation of 9 pc between the centers of two clusters \citep{Qian2012}.
...
%Thus, the separate populations have a CVD power-law index this is larger than the total core population.
...
%%The CVD data points with apparent separation $0\leq L_{2D} \leq 8$  pc can be fitted with a power law of CVD(km/s)$=(0.8\pm 0.002)L_{2D} ({\rm pc})^{0.04\pm 0.001}+(0.002\pm 0.001)$.
%The points in $8\leq L_{2D} \leq 10$  pc (the upturned part) can be fitted with CVD(km/s)$=(0.4\pm 0.03)L_{2D} ({\rm pc})^{0.4\pm 0.02}+(0.01\pm 0.002)$. The turning point (8 pc) is close to the transverse scales of the cube (10 pc). Above this scale, the influence of the thickness is smaller.
...
%We only did a tentative simulation of the case of a thickness of 0.625 pc (1/16) and fit cores in the 2D total intensity map rather than in the 3D data cube.
...
%There are no clear trend of increasing CVD slope with decreasing cloud thickness.

%
%Panel (d) in Figure~\ref{cvd_all} and Figure~\ref{fig:cvd_10pc_y_cvd} show the CVD for the simulations with Mach number 14 (RT1) and 10 (HD1), respectively. As shown in Figure \ref{powersim}, the  RT1 and HD1 runs have slightly different power-law slopes at the times of analysis, where the HD1 run has a slightly flatter power spectrum. This difference is not clearly reflected in the CVD within the errors.
...
%\centering
%\begin{tabular}{c}
%\includegraphics[width=8cm]{sigma_L_data.eps}\\
%\end{tabular}
%\caption{ The solid line denotes the function ($\sigma$/$v_0$)$=\left(\sqrt{(L_{2D}/h)^2+1}\right)^{1/2}$, while the dashed line represents the function ($\sigma$/$v_0$)$=(l_{3D}/h)^{1/2}$. The linewidth $\sigma$ is normalized with a characteristic velocity $v_0$=0.4 km/s, while $L$ is normalized with thickness $h=$0.6 pc. The difference between these two function is plotted with the dash-dotted line. It can be seen that the difference becomes small at large $L_{2D}/h$. The two vertical lines shows the position where $L_{2D}/h=8$ and 10 ($\mathcal{R}=1/8$ and 1/10), respectively. The asterisks shows the linewidth measured at different scales in the real data of Taurus molecular cloud. \label{sigma_L_data}}
%\end{figure}
...
%The two vertical lines shows the position where $L_{2D}/h=8$ and 10 ($\mathcal{R}=1/8$ and 1/10), respectively.
...
%\begin{minipage}[b]{\textwidth}
%\centering
%  \includegraphics[width=8cm]{cvd_10pclog_cvdfield.eps}
%\end{minipage}
%\caption{ The CVD plot for a simulated cloud with a line of sight thickness of 10 pc (with $\mathcal{R}=1$) and a Mach number of 10. The fitting range is 0.2 pc to 2 pc, corresponding to the inertial range of the hydrodynamical simulation (see figure \ref{powersim}).}\label{fig:cvd_10pc_y_cvd}
%\end{figure}
%\appendix

%\begin{figure}[htbp]
%\begin{minipage}[b]{0.45\textwidth}
%  \includegraphics[width=9.2cm]{vdis_fft_errorbar2.eps}
%\end{minipage}
%\caption{Similar plot to Figure~\ref{fig:vdis_all}, for statistical model of cores distributed in a 128$\times$ 128$\times$ 128 grid mentioned in section \ref{subsec:random_cores}.
%%The CVD data points with apparent separation $0\leq L_{2D} \leq 8$  pc can be fitted with a power law of CVD(km/s)$=(0.8\pm 0.002)L_{2D} ({\rm pc})^{0.04\pm 0.001}+(0.002\pm 0.001)$.
%Those points in $8\leq L_{2D} \leq 10$  pc (the upturned part) can be fit with CVD(km/s)$=(0.4\pm 0.03)L_{2D} ({\rm pc})^{0.4\pm 0.02}+(0.01\pm 0.002)$.}\label{fig:vdis_fft_errorbar}
%\end{figure}
